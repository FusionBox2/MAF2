...
#pragma message("BES 25.4.2008 - GENERAL REVISION of medPipeVolumeVR NEEDED")

//----------------------------------------------------------------------------
mafCxxTypeMacro(medPipeVolumeVR_BES);
//----------------------------------------------------------------------------

//----------------------------------------------------------------------------
medPipeVolumeVR_BES::medPipeVolumeVR_BES()
:mafPipe()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::Create(mafSceneNode *n)
//----------------------------------------------------------------------------
{
  Superclass::Create(n);
  ...
  */
  ...
  else 
  {
    ...
    //BES 25.4.2008 - This "Nuovo punto" is probably good working for some volumes
    //but definitely it is not general! 8-bits volumes will be all transparent! Revision is needed.
    //For now, I changed it so 8-bits volumes are visible but ...
  
    if ((m_Maximum - m_Minimum) >= 256)
    {
      /// Nuovo punto
      m_PiecewiseFunction->AddPoint(    0,0.0);
      m_PiecewiseFunction->AddPoint(22737,0.0);
    ...
    else
    {
      ...
      //BES: Color transfer function computed according to "Nuovo punto" CTF
      m_ColorTransferFunction->AddRGBPoint(0, 0.00, 0.00, 0.00);
      m_ColorTransferFunction->AddRGBPoint(45, 0.74, 0.19, 0.14);
      m_ColorTransferFunction->AddRGBPoint(124, 0.96, 0.64, 0.42);
      m_ColorTransferFunction->AddRGBPoint(131, 0.76, 0.78, 0.25);
      m_ColorTransferFunction->AddRGBPoint(162, 1.00, 1.00, 1.00);
      m_ColorTransferFunction->AddRGBPoint(255, 1.00, 1.00, 1.00);
...
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::Show(bool show)
//----------------------------------------------------------------------------
{
  if(!m_Created) return;
...
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::Select(bool sel)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::OnEvent(mafEventBase *maf_event)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::AddPoint(int scalarPoint,double opacity)
//----------------------------------------------------------------------------
{
  m_PiecewiseFunction->AddPoint((double)scalarPoint,opacity);		
}
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::AddPoint(int scalarPoint,double red,double green,double blue)
//----------------------------------------------------------------------------
{
  m_ColorTransferFunction->AddRGBPoint((double)scalarPoint,red,green,blue);
}
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::AddGradPoint(int scalarPoint,double gradient)
//----------------------------------------------------------------------------
{
  m_GradientFunction->AddPoint((double)scalarPoint,gradient);	
}
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::RemoveOpacityPoint(int scalarPoint)
//----------------------------------------------------------------------------
{
  m_PiecewiseFunction->RemovePoint((double)scalarPoint);
}
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::RemoveColorPoint(int scalarPoint)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::RemoveOpacityGradPoint(int scalarPoint)
//----------------------------------------------------------------------------
{
  m_GradientFunction->RemovePoint((double)scalarPoint);	
}
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::SetNumberPoints(int n)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
double medPipeVolumeVR_BES::GetResampleFactor()
//----------------------------------------------------------------------------
{
  return m_ResampleFactor;
}
//----------------------------------------------------------------------------
void medPipeVolumeVR_BES::SetResampleFactor(double value)
//----------------------------------------------------------------------------
{
