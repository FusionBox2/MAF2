...
class MainFramePure : public BaseClass
{
public:
  MainFramePure();

  MainFramePure(wxWindow* parent, wxWindowID id = wxID_ANY, const wxString& title = wxEmptyString, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = DefaultStyle, const wxString& name = wxASCII_STR(wxFrameNameStr));

  ~MainFramePure() override;
...
MainFramePure<BaseFrame, DefaultStyle>::MainFramePure() = default;

template <class BaseFrame, long DefaultStyle>
MainFramePure<BaseFrame, DefaultStyle>::MainFramePure(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style, const wxString& name)
  : MainFramePure()
{
...
template <class BaseFrame, long DefaultStyle>
bool MainFramePure<BaseFrame, DefaultStyle>::Create(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style, const wxString& name)
{
...
template<class BaseClass, long DefaultStyle>
bool AddPane(MainFramePure<BaseClass, DefaultStyle>& frame, wxWindow* window, wxAuiPaneInfo& pane_info)
{
...
template<class BaseClass, long DefaultStyle>
wxWindow* RemovePane(MainFramePure<BaseClass, DefaultStyle>& frame, wxAuiPaneInfo& pane_info)
{
  wxMenuBar* menubar = frame.GetMenuBar();
  int idx = menubar->FindMenu("View");
  if (idx != wxNOT_FOUND)
...
namespace ftk
{
  ...
  {
    return new MainFramePure<wxAuiMDIParentFrame, fbDefaultStyle>(parent, id, title, pos, size, style, name);
  }
}
...
public:
  ...
  MainFramePure<wxAuiMDIParentFrame, ftk::fbDefaultStyle>* m_frame;
...
#include "pic/lhpBuilder/MDICHILD_ICON.xpm"
  ...
  bool App::OnInit()
  {
    ...
    m_logic->m_frame->Bind(wxEVT_MENU, 
      [&](wxCommandEvent& event)
      {
	      auto v = new wxAuiMDIChildFrame(m_logic->m_frame, wxID_ANY, "View");
        auto vr = new wxVTKWindow(v, wxID_ANY);
      	v->Show(true);
      	m_logic->m_frame->Tile();
      },
      MENU_USER_START);
