...
//----------------------------------------------------------------------------
{
	...
	auto pPen = std::make_unique<wxPen>(AXES_COLOR, m_AxisThickness, wxPENSTYLE_SOLID);
	auto pBigPen = std::make_unique<wxPen>(BIG_GRID_COLOR, m_GridThickness, wxPENSTYLE_DOT_DASH);
	auto pSmallPen = std::make_unique<wxPen>(SMALL_GRID_COLOR, m_GridThickness, wxPENSTYLE_DOT);
	pDC->SetPen(*pPen);
...
//----------------------------------------------------------------------------
{
	...
	{
		auto pPen = std::make_unique<wxPen>(wxColor(255, 255, 255), 1, wxPENSTYLE_SOLID);
	...
	auto pFont = std::make_unique<wxFont>(m_TitleFontSize, GetFontFamily(m_TitleFontFamily), wxFONTSTYLE_NORMAL, wxFONTWEIGHT_LIGHT, false, wxEmptyString, wxFONTENCODING_SYSTEM);
	...
	pFont = std::make_unique<wxFont>(m_TickFontSize, GetFontFamily(m_TickFontFamily), wxFONTSTYLE_NORMAL, wxFONTWEIGHT_LIGHT, false, wxEmptyString, wxFONTENCODING_SYSTEM);
	pCompatDC->SetFont(*pFont);
	...
	pCompatDC->SetFont(wxNullFont);
	pFont = std::make_unique<wxFont>(m_TitleFontSize, GetFontFamily(m_TitleFontFamily), wxFONTSTYLE_NORMAL, wxFONTWEIGHT_LIGHT, false, wxEmptyString, wxFONTENCODING_SYSTEM);
	pCompatDC->SetFont(*pFont);
	...
	for (unsigned i = 0; i < m_Graphs.size(); i++)
	{
		...
		{
			auto pPen = std::make_unique<wxPen>(m_ColorTable[varIndex % m_ColorTableNumber], m_CurveThickness, wxPENSTYLE_SOLID);
			pCompatDC->SetPen(*pPen);
		...
		for (unsigned i = 0; i < m_Graphs.size(); i++)
		{
			for (int nJ = 0; nJ < m_Graphs[i]->GetYVarNum(); nJ++, varIndex++)
			{
				...
				if (previnit)
				{
					double valEX;
					double valEY;
					//m_XGraph->GetValueByParam(valEX, m_XGraph->GetIndexX(0), m_XGraph->GetXDer(0));


					//double valX   = m_XGraph->GetValue(nI, m_XGraph->GetIndexX(0), m_XGraph->GetXDer(0));
					//double paramt = m_XGraph->GetValue(nI, 0, 0);
					//unsigned int der = m_Graphs[i]->GetYDer(nJ);
					if (m_XGraph->GetValueByParam(valEX, m_Graphs[i]->GetTime(), m_XGraph->GetIndexX(0), m_XGraph->GetXDer(0)))
					{
						if (m_Graphs[i]->GetValueByParam(valEY, valEX, m_Graphs[i]->GetIndexY(nJ), m_Graphs[i]->GetYDer(nJ)))
						{
							double rEX = (valEX - XMin) / (XMax - XMin);
							double rEY = (YMax - valEY) / (YMax - YMin);
							int nEX = GetIntX(&rcGraphRect, rEX);
							int nEY = GetIntY(&rcGraphRect, rEY);
							pPen = std::make_unique<wxPen>(MARKER_COLOR, 0, wxPENSTYLE_SOLID);
							pCompatDC->SetPen(*pPen);
							...
							rTextRect.SetRight(nEX + MARKER_RADIUS + szTextSize.x + 5);
	...
	else
	{
		...
		for (unsigned i = 0; i < m_Graphs.size(); i++)
		{
			for (unsigned j = 0; j < m_Graphs[i]->GetYVarNum(); j++, varIndex++)
			{
				...
				if (previnit)
				{
					double valEX = m_Graphs[i]->GetTime();
					double valEY;
					if (m_Graphs[i]->GetValueByParam(valEY, valEX, nYNumber, nYDer))
					{
						double rEX = (valEX - XMin) / (XMax - XMin);
						double rEY = (YMax - valEY) / (YMax - YMin);
						int nEX = GetIntX(&rcGraphRect, rEX);
						...
						rTextRect.SetTop(nEY + MARKER_RADIUS);
						rTextRect.SetRight(nEX - MARKER_RADIUS);
						rTextRect.SetBottom(/*nY + MARKER_RADIUS + */m_TitleFntHeight);
						pCompatDC->DrawText(sExp, rTextRect.GetLeft(), rTextRect.GetTop());
						pCompatDC->SetPen(wxNullPen);
					}
...
//----------------------------------------------------------------------------
{
	...
	auto pPopupMenu = std::make_unique<wxMenu>();
...
//----------------------------------------------------------------------------
wxFontFamily mafViewIntGraphWindow::GetFontFamily(mafViewIntGraphFontFamily fFamily)
//----------------------------------------------------------------------------
{
	wxFontFamily nFamily = wxFONTFAMILY_DEFAULT;
	switch (fFamily)
	{
	case mafFF_DEFAULT:
	{
		nFamily = wxFONTFAMILY_DEFAULT;
		break;
	}
...
//----------------------------------------------------------------------------
{
	...
	auto pFile = std::make_unique<wxFile>(sFileName, wxFile::write);
	if (!pFile)
	{
		return (false);
	}
...
{
	return wxRegion(m_dim).Contains(point);
}
