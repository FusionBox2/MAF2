...
//----------------------------------------------------------------------------
mafViewGlobalSlice::mafViewGlobalSlice(const mafString& label, int camera_position, bool show_axes, bool show_grid, bool show_ruler, int stereo)
...
//----------------------------------------------------------------------------
mafView* mafViewGlobalSlice::Copy(mafBaseEventHandler* Listener, bool lightCopyEnabled)
//----------------------------------------------------------------------------
{
	m_LightCopyEnabled = lightCopyEnabled;
	mafViewGlobalSlice* v = new mafViewGlobalSlice(GetLabel(), m_CameraPositionId, m_ShowAxes, m_ShowGrid, m_ShowRuler, m_StereoType);
	...
	if (select && gui)
	{
		...
		if (m_SelectedVolume->m_Pipe)
		{
			...
			if (mafVME::StaticDownCast(node)->GetOutput()->IsA("mafVMEOutputVolume"))
			{
				...
				gui->Enable(ID_OPACITY_SLIDER, true);
				mafPipeVolumeSlice::StaticDownCast(m_SelectedVolume->m_Pipe)->SetTrilinearInterpolation(m_TrilinearInterpolationOn);
				gui->Enable(ID_TRILINEAR_INTERPOLATION_ON, true);
			}
			UpdateGUI();
		}
		else
		{
			gui->Enable(ID_POS_SLIDER, false);
			UpdateGUI();
		}
	}
}
...
//----------------------------------------------------------------------------
{
	...
	if (!pipe_name.empty())
	{
		...
		if (pipe)
		{
			...
			{
				mafPipeVolumeSlice::StaticDownCast(pipe)->InitializeSliceParameters(m_SliceMode, applied_origin, applied_xVector, applied_yVector, true, false);
			}
			else if (pipe_name == _R("mafPipeSurfaceSlice"))
			{
				mafPipeSurfaceSlice::StaticDownCast(pipe)->ShowBoxSelectionOn();
				mafPipeSurfaceSlice::StaticDownCast(pipe)->SetSlice(m_SliceOrigin);
				...
				mafPipeSurfaceSlice::StaticDownCast(pipe)->SetNormal(DoubleNormal);
			}
			else if (pipe_name == _R("mafPipeMeshSlice"))
			{
				...
				mafPipeMeshSlice::StaticDownCast(pipe)->SetNormal(DoubleNormal);
			}
...
//----------------------------------------------------------------------------
{
	...
	if (m_SelectedVolume == n)
	{
		...
		gui->Enable(ID_OPACITY_SLIDER, false);
		UpdateGUI();
	}
	if (m_Rwi && m_BoundsOutlineActor.size() > 0)
	{
...
//----------------------------------------------------------------------------
{
	if (mafEvent* e = mafEvent::SafeDownCast(maf_event))
	{
		...
		{
			auto pipe = mafPipeVolumeSlice::StaticDownCast(m_SelectedVolume->m_Pipe);
			if (pipe)
			{
...
//----------------------------------------------------------------------------
{
	...
	m_GlobalSlider = gui->FloatSlider(ID_POS_SLIDER, _R("pos."), &m_SliderOrigin, m_GlobalBounds[4], m_GlobalBounds[5]);
	gui->Combo(ID_CHANGE_VIEW, _R("view"), &m_ViewIndex, 3, Views);
	m_OpacitySlider = gui->FloatSlider(ID_OPACITY_SLIDER, _R("opacity"), &m_Opacity, 0.1, 1.0);
	...
	gui->Bool(ID_TRILINEAR_INTERPOLATION_ON, _R("Interpolation"), &m_TrilinearInterpolationOn, 1);
	gui->Divider();
	gui->Update();
	return gui;
}
...
//----------------------------------------------------------------------------
{
	...
	for (mafSceneNode* node = m_Sg->GetNodeList(); node; node = node->m_Next)
	{
		if (node->m_Pipe)
		{
			...
			{
				mafPipeSurfaceSlice::StaticDownCast(node->m_Pipe)->SetSlice(m_SliceOrigin);
				double DoubleNormal[3];
				DoubleNormal[0] = (double)m_SliceNormal[0];
				DoubleNormal[1] = (double)m_SliceNormal[1];
				DoubleNormal[2] = (double)m_SliceNormal[2];
				mafPipeSurfaceSlice::StaticDownCast(node->m_Pipe)->SetNormal(DoubleNormal);
			}
			if (node->m_Vme->IsA("mafVMEMesh"))
			{
				mafPipeMeshSlice::StaticDownCast(node->m_Pipe)->SetSlice(m_SliceOrigin);
				double DoubleNormal[3];
				DoubleNormal[0] = (double)m_SliceNormal[0];
				DoubleNormal[1] = (double)m_SliceNormal[1];
				DoubleNormal[2] = (double)m_SliceNormal[2];
