...
//----------------------------------------------------------------------------
{
  ...
  m_TimeBarEntry = new wxTextCtrl  (this, ID_ENTRY, "" , wxDefaultPosition, wxWindow::FromDIP(wxSize(40,17), parent), wxTE_PROCESS_ENTER );
  ...
  else
  {
    ...
    if (mafEvent *e = mafEvent::SafeDownCast(maf_event))
    {
      ...
      if(e->GetId() != TIME_STOP )
        {mafEvent evUnq(this,TIME_SET); evUnq.SetDouble(m_Time); InvokeEvent(evUnq);}
      Update();
    }
  }
}
...
//----------------------------------------------------------------------------
{
  ...
  if (m_Time > time_max)
  {
    ...
    {
      m_Time = time_max;
      {mafEvent evUnq(this, TIME_STOP); OnEvent(&evUnq);}
    }
  }

  {mafEvent evUnq(this, TIME_SET); evUnq.SetDouble(m_Time); evUnq.SetArg(0); InvokeEvent(evUnq);}
  Update();
}
//----------------------------------------------------------------------------
