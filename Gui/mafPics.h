...
public:
	...
	wxBitmap GetBmp(const mafString& id) const;

	/** retrieve a picture from the Factory as a wxImage */
	wxImage GetImg(const mafString& id) const;

	/** retrieve a picture from the Factory as a wxIcon */
	wxIcon GetIcon(const mafString& id) const;
	...
	*/
	...
	wxBitmap GetVmePic(const mafString& id) const;

	/** retrieve the names of the registered vme-pics */
	std::vector<mafString> GetVmeNames() const;
