...
{
	Init(listener, mid, win);
	m_Mode = openfile ? VAL_FILEOPEN : VAL_FILESAVE;
	m_Button = win;
	m_TextCtrl = lab;
	m_MafStringVar = var;
	m_Wildcard = wildcard;
...
bool mafGUIValidator::TransferToWindow()
{
	assert(IsValid());
	wxString s;
	mafString path;
...
bool mafGUIValidator::TransferFromWindow()
{
	bool changed = false;
	return TransferFromWindow(changed);
}
bool mafGUIValidator::TransferFromWindow(bool& changed)
{
	assert(IsValid());
	...
	case VAL_FLOAT_SLIDER_2:
		s = m_TextCtrl->GetValue();
		res = s.ToDouble(&dval);
		if (res && !mafEquals(*m_DoubleVar, dval))
		{
			*m_DoubleVar = dval;
	...
	case VAL_MAF_STRING:
		if (m_MafStringVar)
		{
			s = m_TextCtrl->GetValue();
			res = true;
			if (res && (mafWxToString(s) != *m_MafStringVar))
			{
...
void mafGUIValidator::OnChar(wxKeyEvent& event)
{
		...
		)
	{
		...
		// i.e. console widget
		TransferFromWindow();
		{ mafEvent evUnq(m_TextCtrl, m_ModuleId); InvokeEvent(evUnq); }
		return; // eat message
	}
...
void mafGUIValidator::OnKillFocus(wxFocusEvent& event)
{
		...
		m_Mode == VAL_SLIDER_2)
	{
		if (IsValid())
		{
			...
			TransferFromWindow(send_message);
			if (send_message)
			{
				mafEvent evUnq(m_TextCtrl, m_ModuleId); InvokeEvent(evUnq);
			}
...
void mafGUIValidator::OnScrollEvent(wxScrollEvent& event)
{
	...
	if (IsValid())
	{
		...
		case VAL_SLIDER:
			...
			s.Printf("%d", *m_IntVar);
			m_TextCtrl->SetValue(s);
			{ mafEvent evUnq(m_Slider, m_ModuleId); InvokeEvent(evUnq); }
			break;
		case VAL_FLOAT_SLIDER:
			TransferFromWindow();
			s.Printf("%g", *m_DoubleVar);
...
void mafGUIValidator::OnCommandEvent(wxCommandEvent& event)
{
	if (IsValid())
	{
		...
		{
		case VAL_CHECKBOX:
		{ mafEvent evUnq(m_CheckBox, m_ModuleId); InvokeEvent(evUnq); }
		break;
		case VAL_RADIOBOX:
		{ mafEvent evUnq(m_RadioBox, m_ModuleId); InvokeEvent(evUnq); }
		break;
		case VAL_COMBOBOX:
		{ mafEvent evUnq(m_ComboBox, m_ModuleId); InvokeEvent(evUnq); }
		break;
		}
	}
	event.Skip();
...
void mafGUIValidator::OnButton(wxCommandEvent& event)
{
	...
	if (IsValid())
	{
		...
		case VAL_FILESAVE:
		{
			...
			if (ret_code == wxID_OK)
			{
				*m_MafStringVar = mafWxToString(dialog.GetPath());
		...
		if (!cancel)
		{
			mafEvent evUnq(m_Button, m_ModuleId); InvokeEvent(evUnq);
		}
	}
}
//----------------------------------------------------------------------------
