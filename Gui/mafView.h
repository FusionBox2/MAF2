...
public:
	...
	virtual vtkRenderWindow* GetRWI() { return nullptr; }
	...
	virtual const mafString& GetHTMLText() { return m_HTMLText; }
...
protected:
	...
	vtkCellPicker* m_Picker2D = nullptr;  ///< the picker used to pick the in the render window
	vtkMAFRayCast3DPicker* m_Picker3D = nullptr; ///< Used to pick in a VTK Render window

	mafVME* m_PickedVME = nullptr;   ///< Pointer to the picked vme. It is initialized on picking
	vtkProp3D* m_PickedProp = nullptr;  ///< Pointer to the picked vme Prop3D. It is initialized on picking
	double         m_PickedPosition[3];
	int            m_NumberOfVisibleVme = 0; ///< perform ResetCamera only for the first vme shown into the view
