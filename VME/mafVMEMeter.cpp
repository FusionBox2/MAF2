...
//-------------------------------------------------------------------------
{
	...
	gui->Divider();
	gui->Combo(ID_METER_MODE, _L("mode"), &(GetMeterAttributes()->m_MeterMode), num_mode, mode_choices_string, _L("Choose the meter mode"));
	gui->Divider();
	...
	gui->Button(ID_START2_METER_LINK, &m_StartVme2Name, _L("Start2"), _L("Select the start vme for the second line"));
	gui->Button(ID_END2_METER_LINK, &m_EndVme2Name, _L("End 2"), _L("Select the vme representing \nthe point for line distance"));

	gui->Bool(ID_INFINITE_LINE, _L("Infinite"), &m_InfiniteLine);
	gui->Bool(ID_LINE_ANGLE2, _L("2 lines angle"), &m_LineAngle2);

	if (GetMeterAttributes()->m_MeterMode == POINT_DISTANCE)
		gui->Enable(ID_END2_METER_LINK, false);
	gui->Enable(ID_INFINITE_LINE, GetMeterAttributes()->m_MeterMode == LINE_DISTANCE);
	gui->Enable(ID_LINE_ANGLE2, GetMeterAttributes()->m_MeterMode == LINE_ANGLE || GetMeterAttributes()->m_MeterMode == LINE_DISTANCE);
	gui->Enable(ID_START2_METER_LINK, GetMeterAttributes()->m_MeterMode == LINE_ANGLE && m_LineAngle2);

	gui->Bool(ID_PLOT_PROFILE, _L("plot profile"), &m_GenerateHistogram);
	gui->Enable(ID_PLOT_PROFILE, GetMeterAttributes()->m_MeterMode == POINT_DISTANCE);

	gui->Button(ID_PLOTTED_VME_LINK, &m_ProbeVmeName, _L("Probed"), _L("Select the vme that will be plotted"));
	gui->Enable(ID_PLOTTED_VME_LINK, GetMeterAttributes()->m_MeterMode == POINT_DISTANCE);
...
//-------------------------------------------------------------------------
{
	...
	if (mafEvent* e = mafEvent::SafeDownCast(maf_event))
	{
		...
		{
		case ID_LINE_ANGLE2:
			gui->Enable(ID_START2_METER_LINK, m_LineAngle2 != 0);
			this->Modified();
		...
		case ID_METER_MODE:
		{
			...
			{
				gui->Enable(ID_END2_METER_LINK, false);
				gui->Enable(ID_LINE_ANGLE2, false);
				gui->Enable(ID_START2_METER_LINK, false);
			}
			else if (GetMeterAttributes()->m_MeterMode == LINE_DISTANCE)
			{
				gui->Enable(ID_END2_METER_LINK, true);
				gui->Enable(ID_LINE_ANGLE2, true);
				gui->Enable(ID_START2_METER_LINK, false);
			}
