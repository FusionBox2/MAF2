...
//-----------------------------------------------------------------------
void mafVMEBSplineSurface::InternalStore(mafStorageElementBuilder& parent)
//-----------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------
void mafVMEBSplineSurface::InternalRestore(const mafStorageElement& node)
//-----------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
const char** mafVMEBSplineSurface::GetIcon()
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
{
	...
	gui->Button(ID_PNTS_GROUP_LINK, &m_PointsGroupName, _L("Points"), _L("Select the Points cloud to create the Spline"));
	gui->Integer(ID_ORDERX, _L("Order X"), &m_OrderX, 1, 10);
	gui->Integer(ID_ORDERY, _L("Order Y"), &m_OrderY, 1, 10);
	gui->Combo(ID_MODE, _L("Mode"), &m_Mode, DIM(mode_choices_string), mode_choices_string);
	gui->Combo(ID_SUBMODEU, _L("Submode X"), &m_SubModeU, DIM(submode_choices_string), submode_choices_string);
	gui->Combo(ID_SUBMODEV, _L("Submode Y"), &m_SubModeV, DIM(submode_choices_string), submode_choices_string);

	gui->Bool(ID_USEBSLINES, _L("Use BSLines"), &m_useBSLines);

	gui->FloatSlider(ID_SMOOTHX, _L("Smooth X"), &m_SmoothX, 0.0, 1000.0);
	gui->FloatSlider(ID_SMOOTHY, _L("Smooth Y"), &m_SmoothY, 0.0, 1000.0);

	gui->Integer(ID_NUMTESSELX, _L("Num parts X"), &m_TesselX, 2, 500);
	gui->Integer(ID_NUMTESSELY, _L("Num parts Y"), &m_TesselY, 2, 500);

	gui->Enable(ID_SMOOTHX, m_Mode == 2);
	gui->Enable(ID_SMOOTHY, m_Mode == 2);

	gui->Enable(ID_SUBMODEU, m_Mode == BCurve<double>::SPT_MODE_INTERP);
	gui->Enable(ID_SUBMODEV, m_Mode == BCurve<double>::SPT_MODE_INTERP);
