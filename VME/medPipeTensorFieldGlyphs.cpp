...
//----------------------------------------------------------------------------
{
	...
	if (nTensors == 0)  //no tensor field available, this pipe does not work
		bSizerMain->Add(new wxStaticText(gui, wxID_ANY, _("No tensor field to visualize.")));
	else
	{
		if (nTensors > 1)
		{
			wxBoxSizer* bSizer1 = new wxBoxSizer(wxHORIZONTAL);
			bSizer1->Add(new wxStaticText(gui, wxID_ANY, _("Tensor Field:"),
				wxDefaultPosition, wxSize(65, -1), 0), 0, wxALL, 5);

			wxComboBox* comboField = new wxComboBox(gui, ID_TENSORFIELD,
