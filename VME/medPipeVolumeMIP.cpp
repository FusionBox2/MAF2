...
//----------------------------------------------------------------------------
{
	...
	gui->Lut(ID_LUT_CHOOSER, _L("lut"), m_ColorLUT);
	UpdateMIPFromLUT();

	gui->Double(ID_RESAMPLE_FACTOR, _R("Resample Factor"), &m_ResampleFactor, 0.0, 1.0, 2, _R("Insert a value for the resampling factor"));
	gui->Divider();
	return gui;
}
//----------------------------------------------------------------------------
