...
//-------------------------------------------------------------------------
const char** mafVMEBSplineLine::GetIcon()
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
{
	...
	gui->Button(ID_PNTS_CLOUD_LINK, &m_PointsCloudName, _L("Points"), _L("Select the Points cloud to create the Spline"));
	gui->Label(_R("Points: "), &m_PointsCloudName);
	gui->Integer(ID_ORDER, _L("Order"), &m_Order, 1, 10);
	gui->Combo(ID_MODE, _L("Mode"), &m_Mode, DIM(mode_choices_string), mode_choices_string);
	gui->Combo(ID_SUBMODE, _L("Submode"), &m_SubMode, DIM(submode_choices_string), submode_choices_string);
	gui->FloatSlider(ID_SMOOTH, _L("Smooth"), &m_Smooth, 0.0, 1000.0);
	gui->Integer(ID_NUMTESSEL, _L("Num parts"), &m_TesselSize, 2, 500);

	gui->Enable(ID_SMOOTH, m_Mode == 2);
	gui->Bool(ID_FILTER, _L("Filter ident"), &m_FilterIdent);
	gui->Bool(ID_EXTRACT, _L("Extract fibre"), &m_ExtractFibre);
	gui->Enable(ID_EXTRACT, m_FilterIdent != 0);
	gui->Enable(ID_SUBMODE, m_Mode == BCurve<double>::SPT_MODE_INTERP);
