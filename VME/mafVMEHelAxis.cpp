...
//-----------------------------------------------------------------------
void mafVMEHelAxis::InternalStore(mafStorageElementBuilder& parent)
//-----------------------------------------------------------------------
{
	Superclass::InternalStore(parent);
	parent[_R("Transform")].SetValue(m_Transform->GetMatrix());
	//code for backward compatibility
	parent[_R("ScaleFactor")].SetValue(m_LengthFactor);
	parent[_R("RadiusFactor")].SetValue(m_RadiusFactor);
	parent[_R("LengthFactor")].SetValue(m_LengthFactor);
	parent[_R("MinAngle")].SetValue(m_MinAngle);
	parent[_R("MinTime")].SetValue(m_MinTime);
	parent[_R("MaxTime")].SetValue(m_MaxTime);
	parent[_R("Mode")].SetValue(m_Mode);
	parent[_R("AligningMode")].SetValue(m_AligningMode);
}

//-----------------------------------------------------------------------
void mafVMEHelAxis::InternalRestore(const mafStorageElement& node)
//-----------------------------------------------------------------------
{
	Superclass::InternalRestore(node);
	m_Transform->SetMatrix(node[_R("Transform")].As<mafMatrix>());
	//code for backward compatibility
	m_LengthFactor = node[_R("ScaleFactor")].As<double>();
	m_RadiusFactor = m_LengthFactor;
	m_RadiusFactor = node[_R("RadiusFactor")].As<double>();
	m_LengthFactor = node[_R("LengthFactor")].As<double>();
	m_MinAngle = node[_R("MinAngle")].As<double>();
	m_MinTime = node[_R("MinTime")].As<double>();
	m_MaxTime = node[_R("MaxTime")].As<double>();
	m_Mode = node[_R("Mode")].As<int>();
	m_AligningMode = node[_R("AligningMode")].As<int>();
	m_MeanChanges = 1;
...
//----------------------------------------------------------------------------
{
	...
	case ID_MODE:
	{
		m_MeanChanges = 1;
		auto gui = AccessGUI();
		gui->Enable(ID_MIN_TIME, m_Mode == 1);
		gui->Enable(ID_MAX_TIME, m_Mode == 1);
		gui->Enable(ID_REF_TIME, m_Mode == 2);
		Modified();
		mafEvent cam_event(this, CAMERA_UPDATE);
		this->ForwardUpEvent(cam_event);
		break;
...
//----------------------------------------------------------------------------
{
	...
	gui->Show(false);
	gui->Double(ID_RADIUS_FACTOR, _L("radius scale"), &m_RadiusFactor);
	gui->Double(ID_LENGTH_FACTOR, _L("length scale"), &m_LengthFactor);
	gui->Double(ID_MIN_ANGLE, _L("Min angle"), &m_MinAngle, 0.0);
	gui->Combo(ID_MODE, _L("Mode"), &m_Mode, 3, mode_choices, _L("Select mode"));
	gui->Combo(ID_ALIGNING, _L("Align"), &m_AligningMode, 7, align_choices, _L("Select aligning"));

	gui->Label(_L("Proximal :"), true);
	...
	gui->Label(_L("Distal :"), true);
	if (mafVME* vme = GetDistal())
		m_DistalName = vme->GetName();
	gui->Label(&m_DistalName);
	gui->Button(ID_DISTAL, _L("Distal"));
	gui->Button(ID_RESETDISTAL, _L("Reset distal"));

	gui->Double(ID_MIN_TIME, _L("Min time"), &m_MinTime, 0.0);
	gui->Double(ID_MAX_TIME, _L("Max time"), &m_MaxTime);
	gui->Double(ID_REF_TIME, _L("Ref time"), &m_RefTime);
	gui->Divider();
	gui->Enable(ID_MIN_TIME, m_Mode == 1);
	gui->Enable(ID_MAX_TIME, m_Mode == 1);
	gui->Enable(ID_REF_TIME, m_Mode == 2);
	gui->Divider();
	gui->Label(_R("Direction:"));
	m_StrDir[0] = mafString::Format(_R("x: %f"), m_Direction[0]);
	m_StrDir[1] = mafString::Format(_R("y: %f"), m_Direction[1]);
	m_StrDir[2] = mafString::Format(_R("z: %f"), m_Direction[2]);
	gui->Label(_R(""), &m_StrDir[0]);
	gui->Label(_R(""), &m_StrDir[1]);
	gui->Label(_R(""), &m_StrDir[2]);
	gui->Label(_R("Rotation Center:"));
	m_StrPnt[0] = mafString::Format(_R("x: %f"), m_StartPoint[0]);
	m_StrPnt[1] = mafString::Format(_R("y: %f"), m_StartPoint[1]);
	m_StrPnt[2] = mafString::Format(_R("z: %f"), m_StartPoint[2]);
	gui->Label(_R(""), &m_StrPnt[0]);
	gui->Label(_R(""), &m_StrPnt[1]);
	gui->Label(_R(""), &m_StrPnt[2]);
	gui->Label(_R("Angle:"));
	m_StrAng = (m_Mode == 1) ? _R("") : mafString::Format(_R("%f"), m_Angle);
	gui->Label(_R(""), &m_StrAng);
	gui->Label(_R("Translation:"));
	m_StrTrl = (m_Mode == 1) ? _R("") : mafString::Format(_R("%f"), m_Translation);
	gui->Label(_R(""), &m_StrTrl);
	gui->Update();
	return gui;
}
