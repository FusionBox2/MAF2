...
//-----------------------------------------------------------------------
void medVMELabeledVolume::InternalStore(mafStorageElementBuilder& parent)
//-----------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------
void medVMELabeledVolume::InternalRestore(const mafStorageElement& node)
//-----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
	...
	gui->Button(ID_INSERT_LABEL, _L("Add label"), _R(""), _L("Add a label"));
	gui->Button(ID_REMOVE_LABEL, _L("Remove label"), _R(""), _L("Remove a label"));
	gui->Button(ID_EDIT_LABEL, _L("Edit label"), _R(""), _L("Edit a label"));
	if (m_VolumeLink)
	{
		EnableWidgets(true);
	}
	...
	gui->Divider(2);
	m_LabelCheckBox = gui->CheckList(ID_LABELS, _L("Labels"), 360, _L("Chose label to visualize"));
...
//----------------------------------------------------------------------------
{
	if (auto gui = AccessGUI())
	{
		...
		{
			gui->Enable(ID_INSERT_LABEL, enable);
			gui->Enable(ID_REMOVE_LABEL, enable);
			gui->Enable(ID_EDIT_LABEL, enable);
		}
		else
		{
			int noc = m_TagLabel->GetNumberOfComponents();
			...
			gui->Enable(ID_INSERT_LABEL, enable);
			gui->Enable(ID_REMOVE_LABEL, labelPresent);
			gui->Enable(ID_EDIT_LABEL, labelPresent);
		}
		gui->Update();
	}
}
