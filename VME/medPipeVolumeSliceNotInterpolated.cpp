...
//----------------------------------------------------------------------------
{
	...
	if (m_DataType == VTK_IMAGE_DATA)
	{
		...
		for (m_CurrentImageIndex = 0; m_CurrentImageIndex < m_Slicer->GetNumberOfOutputPorts(); m_CurrentImageIndex++)
		{
			m_SlicerOutputImageData.at(m_CurrentImageIndex) = (vtkImageData*)m_Slicer->GetOutput(m_CurrentImageIndex);
...
//----------------------------------------------------------------------------
{
	...
	m_ImageMapToColors.at(m_CurrentImageIndex)->SetInputConnection(m_ImageShiftScale.at(m_CurrentImageIndex)->GetOutputPort());
...
//----------------------------------------------------------------------------
{
	...
	if (m_ShowGui)
	{
		gui->Lut(ID_LUT, _R("LUT"), m_VolumeLUT); // Lut widget
		mafString choices[3] = { _R("X"),_R("Y"),_R("Z") };
		gui->Combo(ID_AXIS, _R("Axis"), &m_SliceAxis, 3, choices); // Slice Axis
		m_SliceSlider = gui->FloatSlider(ID_SLICE, _R("Slice"), &m_CurrentSlice, m_Bounds[m_SliceAxis * 2], m_Bounds[(m_SliceAxis * 2) + 1]); // Current slice coordinate
		gui->Divider();
	}
	return gui;
}
