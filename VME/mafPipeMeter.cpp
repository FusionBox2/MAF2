...
//----------------------------------------------------------------------------
{
	...
	gui->Enable(ID_DISTANCE_RANGE, meter_attrib->m_ColorMode == mafVMEMeter::RANGE_COLOR);
	gui->Enable(ID_TUBE_RADIUS, meter_attrib->m_Representation == mafVMEMeter::TUBE_REPRESENTATION);
	gui->Enable(ID_TUBE_CAPPING, meter_attrib->m_Representation == mafVMEMeter::TUBE_REPRESENTATION);
	gui->Divider();
...
//----------------------------------------------------------------------------
{
	...
	if (linked_vme && linked_vme->IsMAFType(mafVMELandmarkCloud) && m_MeterVME->GetLinkSubId(_R("StartVME")) != -1)
	{
		mafVMELandmarkCloud::StaticDownCast(linked_vme)->GetLandmark(m_MeterVME->GetLinkSubId(_R("StartVME")), pos, -1);
		TmpTransform->SetMatrix(*linked_vme->GetOutput()->GetAbsMatrix());
		TmpTransform->TransformPoint(pos, pos);
	}
	else if (linked_vme)
