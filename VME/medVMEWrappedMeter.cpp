	...
	if (GetMeterMode() == medVMEWrappedMeter::POINT_DISTANCE)
	{
		...
		if (start_vme && end_vme && result)
		{
			for (int j = 0; j < m_OrderMiddlePointsNameVMEList.size(); j++)
			{
				...
				{
					((mafVMELandmarkCloud*)middleVME)->GetLandmark(m_OrderMiddlePointsNameVMEList[j], m_MiddlePointList[m_MiddlePointList.size() - 1], currTs);
					mafMatrix tm;
					((mafVME*)middleVME)->GetOutput()->GetAbsMatrix(tm, currTs);
					m_TmpTransform->SetMatrix(tm);
				...
				else
				{
					...
					((mafVME*)middleVME)->GetOutput()->GetAbsPose(m_MiddlePointList[m_MiddlePointList.size() - 1], orientation, currTs);
				}
	...
	/*  else if (GetMeterMode() == medVMEWrappedMeter::LINE_DISTANCE)
	  {
		...
		if (start_vme && end1_vme && end2_vme)
		{
		  ...
		  {
			((mafVMELandmarkCloud *)end1_vme)->GetLandmark(GetLinkSubId(_R("EndVME1")),m_EndPoint,-1);
		  ...
		  {
			((mafVMELandmarkCloud *)end2_vme)->GetLandmark(GetLinkSubId(_R("EndVME2")),m_EndPoint2,-1);
...
//-------------------------------------------------------------------------
{
	...
	gui->Divider();
	gui->Combo(ID_WRAPPED_METER_MODE, _L("wrap"), &m_WrappedMode, num_wrap, wrap_choices_string, _L("Choose the meter mode"));
	...
	gui->Button(ID_END1_METER_LINK, &m_EndVme1Name, _L("End 1"), _L("Select the end vme for point distance"));
	...
	gui->Button(ID_WRAPPED_METER_LINK, &m_WrappedVmeName, _L("Wrapped"), _L("Select the vme representing Vme to be wrapped"));
