...
//----------------------------------------------------------------------------
void mafPipeMeshSlice::Create(mafNode* node, mafView* view)
//----------------------------------------------------------------------------
{
...
{
	m_Vme->Update();
	m_Vme->GetOutput()->GetVTKOutputPort()->GetProducer()->Update();
...
//----------------------------------------------------------------------------
{
	...
	gui->FloatSlider(ID_BORDER_CHANGE, _L("Thickness"), &m_Border, 1.0, 5.0);
	gui->Enable(ID_BORDER_CHANGE, m_Wireframe);
	gui->Divider(2);

	gui->Bool(ID_WIRED_ACTOR_VISIBILITY, _L("Element Edges"), &m_BorderElementsWiredActor, 1);
	gui->Enable(ID_WIRED_ACTOR_VISIBILITY, !m_Wireframe);

	gui->Divider(2);
	gui->Bool(ID_USE_VTK_PROPERTY, _L("Property"), &m_UseVTKProperty, 1);
	...
	m_LutSwatch = gui->Lut(ID_LUT, _R("Lut"), m_Table);
...
//----------------------------------------------------------------------------
{
	...
	if (auto gui = AccessGUI())
	{
		...
		gui->Enable(ID_WIRED_ACTOR_VISIBILITY, false);
		UpdateGUI();
	}
	{ mafEvent evUnq(this, CAMERA_UPDATE); InvokeEvent(evUnq); }
}
...
//----------------------------------------------------------------------------
{
	...
	if (auto gui = AccessGUI())
	{
		...
		gui->Enable(ID_WIRED_ACTOR_VISIBILITY, true);
		UpdateGUI();
	}
	{ mafEvent evUnq(this, CAMERA_UPDATE); InvokeEvent(evUnq); }
}
...
//----------------------------------------------------------------------------
{
	...
	for (int i = 0; i < n; i++)
	{
		double* rgba = table->GetTableValue(i);
