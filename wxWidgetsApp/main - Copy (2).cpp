...
{
public:
    ftkDocument(ftkDocument* parent = nullptr);
    virtual ~ftkDocument();
...
protected:
    ...
    ftkDocTemplate* m_documentTemplate;
...
private:
    ...
    std::list<ftkDocument*> m_childDocuments;

    wxDECLARE_ABSTRACT_CLASS(ftkDocument);
    wxDECLARE_NO_COPY_CLASS(ftkDocument);
};

class WXDLLIMPEXP_CORE ftkView : public wxEvtHandler
{
public:
    ftkView();
    virtual ~ftkView();
...
protected:
    ...
    ftkDocument* m_viewDocument;
    wxString          m_viewTypeName;
    wxWindow* m_viewFrame;

    ftkDocChildFrameAnyBase* m_docChildFrame;

private:
    wxDECLARE_ABSTRACT_CLASS(ftkView);
    wxDECLARE_NO_COPY_CLASS(ftkView);
};

class TextEditDocument : public ftkDocument
{
public:
    TextEditDocument() : ftkDocument() {}
...
{
public:
    TextEditView() : ftkView(), m_text(nullptr) {}

    virtual bool OnCreate(ftkDocument* doc, long flags) override;
...
public:
    ...
    // creation
    virtual bool InitDocument(ftkDocument* doc,
        const wxString& path,
        long flags = 0);
    ...
    ftkDocManager* GetDocumentManager() const { return m_documentManager; }
    void SetDocumentManager(ftkDocManager* manager)
    {
...
protected:
    ...
    wxString          m_viewTypeName;
    ftkDocManager* m_documentManager;
...
public:
    ...
    virtual ftkDocTemplate* SelectDocumentPath(ftkDocTemplate** templates,
        int noTemplates, wxString& path, long flags, bool save = false);
    virtual ftkDocTemplate* SelectDocumentType(ftkDocTemplate** templates,
        int noTemplates, bool sort = false);
    virtual ftkDocTemplate* SelectViewType(ftkDocTemplate** templates,
        int noTemplates, bool sort = false);
    virtual ftkDocTemplate* FindTemplateForPath(const wxString& path);

    void AssociateTemplate(ftkDocTemplate* temp);
...
protected:
    ...
    wxDECLARE_EVENT_TABLE();
    wxDECLARE_DYNAMIC_CLASS(ftkDocManager);
    wxDECLARE_NO_COPY_CLASS(ftkDocManager);
};
...
private:
    ...
    wxDECLARE_NO_COPY_CLASS(ftkDocChildFrameAnyBase);
};
...
public:
    ...
    // ctor for a frame showing the given view of the specified document
    wxDocChildFrameAny(ftkDocument* doc,
        ftkView* view,
        ParentFrame* parent,
        wxWindowID id,
        const wxString& title,
...
using wxDocChildFrame = wxDocChildFrameAny<wxFrame, wxFrame>;
...
using ftkDocParentFrame = wxDocParentFrameAny<wxFrame>;
...
private:
    ...
    wxDECLARE_NO_COPY_CLASS(ftkDocPrintout);
};
#endif // wxUSE_PRINTING_ARCHITECTURE
...
wxIMPLEMENT_ABSTRACT_CLASS(ftkDocument, wxEvtHandler);
wxIMPLEMENT_ABSTRACT_CLASS(ftkView, wxEvtHandler);
wxIMPLEMENT_ABSTRACT_CLASS(ftkDocTemplate, wxObject);
wxIMPLEMENT_DYNAMIC_CLASS(ftkDocManager, wxEvtHandler);
...
ftkDocument::ftkDocument(ftkDocument* parent)
{
...
bool ftkDocument::Close()
{
    ...
    {
        ftkDocument* const childDoc = m_childDocuments.front();
...
{
    ftkDocManager* manager = GetDocumentManager();
    ...
    else // have views
    {
        ...
        for (;; )
        {
            ftkView* view = (ftkView*)*m_documentViews.begin();
...
ftkView* ftkDocument::GetFirstView() const
{
    ...
    return static_cast<ftkView*>(m_documentViews.GetFirst()->GetData());
}
...
bool ftkDocument::SaveAs()
{
    ...
    if (docTemplate->GetViewClassInfo() && docTemplate->GetDocClassInfo())
    {
        ...
        while (node)
        {
            ftkDocTemplate* t = (ftkDocTemplate*)node->GetData();
...
wxCommandProcessor* ftkDocument::OnCreateCommandProcessor()
{
...
void ftkDocument::UpdateAllViews(ftkView* sender, wxObject* hint)
{
    ...
    while (node)
    {
        ftkView* view = (ftkView*)node->GetData();
        if (view != sender)
...
ftkDocTemplate::ftkDocTemplate(ftkDocManager* manager,
    const wxString& descr,
    const wxString& filter,
    const wxString& dir,
    const wxString& ext,
...
ftkDocument* ftkDocTemplate::CreateDocument(const wxString& path, long flags)
{
    ...
    ftkDocument* const doc = DoCreateDocument();
...
ftkView* ftkDocTemplate::CreateView(ftkDocument* doc, long flags)
{
    std::unique_ptr<ftkView> view(DoCreateView());
    if (!view)
        return nullptr;
...
ftkDocument* ftkDocTemplate::DoCreateDocument()
{
    ...
    return static_cast<ftkDocument*>(m_docClassInfo->CreateObject());
}

ftkView* ftkDocTemplate::DoCreateView()
{
    if (!m_viewClassInfo)
        return nullptr;

    return static_cast<ftkView*>(m_viewClassInfo->CreateObject());
}
...
bool ftkDocManager::CloseDocuments(bool force)
{
    ...
    while (node)
    {
        ftkDocument* doc = (ftkDocument*)node->GetData();
...
bool ftkDocManager::Clear(bool force)
{
    ...
    while (node)
    {
        ftkDocTemplate* templ = (ftkDocTemplate*)node->GetData();
        wxList::compatibility_iterator next = node->GetNext();
        delete templ;
        node = next;
    }
...
ftkView* ftkDocManager::GetAnyUsableView() const
{
    ...
    if (!view && !m_docs.empty())
    {
        ...
        if (!node->GetNext())
        {
            auto doc = static_cast<ftkDocument*>(node->GetData());
...
namespace
{
    ...
    std::vector<ftkDocTemplate*> GetVisibleTemplates(const wxList& allTemplates)
    {
        ...
        if (totalNumTemplates)
        {
            ...
            for (wxList::const_iterator i = allTemplates.begin(),
                ...
                ++i)
            {
                ftkDocTemplate* const temp = (ftkDocTemplate*)*i;
                if (temp->IsVisible())
...
ftkDocument* ftkDocManager::FindDocumentByPath(const wxString& path) const
{
    ...
    {
        ftkDocument* const doc = wxStaticCast(*i, ftkDocument);
...
ftkDocument* ftkDocManager::CreateDocument(const wxString& pathOrig, long flags)
{
    ...
    std::vector<ftkDocTemplate*> templates(GetVisibleTemplates(m_templates));
    ...
    {
        if (!CloseDocument((ftkDocument*)GetDocuments().GetFirst()->GetData()))
        {
    ...
    ftkDocument* const docNew = temp->CreateDocument(path, flags);
    if (!docNew)
        return nullptr;
...
ftkView* ftkDocManager::CreateView(ftkDocument* doc, long flags)
{
    ...
    auto view = temp->CreateView(doc, flags);
    if (view)
...
wxCommandProcessor* ftkDocManager::GetCurrentCommandProcessor() const
{
    ftkDocument* const doc = GetCurrentDocument();
...
ftkDocTemplate* ftkDocManager::FindTemplateForPath(const wxString& path)
{
    ...
    {
        ftkDocTemplate* temp = (ftkDocTemplate*)m_templates.Item(i)->GetData();
...
ftkDocTemplate* ftkDocManager::SelectDocumentPath(ftkDocTemplate** templates,
    int noTemplates,
    wxString& path,
    long WXUNUSED(flags),
    bool WXUNUSED(save))
...
ftkDocTemplate* ftkDocManager::SelectDocumentType(ftkDocTemplate** templates,
    int noTemplates, bool sort)
{
    wxArrayString strings;
    wxScopedArray<ftkDocTemplate*> data(noTemplates);
    int i;
    int n = 0;
    ...
    default:
        ...
        theTemplate = (ftkDocTemplate*)wxGetSingleChoiceData
        (
...
ftkDocTemplate* ftkDocManager::SelectViewType(ftkDocTemplate** templates,
    int noTemplates, bool sort)
{
    wxArrayString strings;
    wxScopedArray<ftkDocTemplate*> data(noTemplates);
    ...
    for (i = 0; i < noTemplates; i++)
    {
        ftkDocTemplate* templ = templates[i];
...
void ftkDocManager::AssociateTemplate(ftkDocTemplate* temp)
{
...
ftkDocTemplate* ftkDocManager::FindTemplate(const wxClassInfo* classinfo)
{
    for (wxList::compatibility_iterator node = m_templates.GetFirst();
        node;
        node = node->GetNext())
    {
        ftkDocTemplate* t = wxStaticCast(node->GetData(), ftkDocTemplate);
...
using ftkDocMDIParentFrame = wxDocParentFrameAny<wxMDIParentFrame> ;
using ftkDocMDIChildFrame = wxDocChildFrameAny<wxMDIChildFrame, wxMDIParentFrame>;
...
bool MyApp::OnInit()
{
    ...
    ftkDocManager* docManager = new ftkDocManager;
    ...
    else // multiple documents mode: allow documents of different types
    {
        ...
        new ftkDocTemplate(docManager, "Text", "*.txt;*.text", "", "txt;text",
            "Text Doc", "Text View",
...
int MyApp::OnExit()
{
    ftkDocManager* const manager = ftkDocManager::GetDocumentManager();
#if wxUSE_CONFIG
...
wxFrame* MyApp::CreateChildFrame(ftkView* view, bool isCanvas)
{
...
wxIMPLEMENT_DYNAMIC_CLASS(TextEditDocument, ftkDocument);
...
wxIMPLEMENT_DYNAMIC_CLASS(TextEditView, ftkView);
