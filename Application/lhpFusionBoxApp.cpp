...
#include "pic/lhpBuilder/MDICHILD_ICON.xpm"
	...
	auto ArbitraryView = std::make_unique<mafViewArbitrarySlice>(_R("Arbitrary"));
	ArbitraryView->PackageView();
	m_Logic->Plug(std::move(ArbitraryView), view_visibility);


	// View DRR
	auto vdrr = std::make_unique<mafViewVTK>(_R("DRR"));
	...
	m_Logic->Plug(std::move(vdrr), view_visibility);

	// View Analog graph
	auto graph = std::make_unique<mafViewVTK>(_R("Analog Graph"), CAMERA_PERSPECTIVE, false);
	graph->PlugVisualPipe(_R("medVMEAnalog"), _R("medPipeGraph"), MUTEX);
	m_Logic->Plug(std::move(graph)/*, view_visibility*/);

	//View Global Slice
	auto GlobalSlice = std::make_unique<mafViewGlobalSliceCompound>(_R("Global Slice"));
	GlobalSlice->PackageView();
	m_Logic->Plug(std::move(GlobalSlice), view_visibility);

	auto viso = std::make_unique<mafViewVTK>(_R("Isosurface"));
	...
	m_Logic->Plug(std::move(viso), view_visibility);

	auto visoGPU = std::make_unique<mafViewVTK>(_R("Isosurface (GPU)"));
	...
	m_Logic->Plug(std::move(visoGPU), view_visibility);

	auto viewOrthoSlice = std::make_unique<mafViewOrthoSlice>(_R("OrthoSlice"));
	viewOrthoSlice->PackageView();
	m_Logic->Plug(std::move(viewOrthoSlice)/*, view_visibility*/);

	auto vrxctl = std::make_unique<mafViewRXCT>(_R("RXCT"));
	vrxctl->PackageView();
	m_Logic->Plug(std::move(vrxctl), view_visibility);

	auto vsurface = std::make_unique<mafViewVTK>(_R("Surface"));
	...
	m_Logic->Plug(std::move(vsurface));

	auto vsurfacet = std::make_unique<mafViewVTK>(_R("Textured Surface"));
	...
	m_Logic->Plug(std::move(vsurfacet));

	auto vgraph = std::make_unique<mafViewIntGraph>(_R("Biomechanical graph"));
	...
	lhpOpExporterCSVGraph::m_ViewIntGraph = vgraph.get();
	m_Logic->Plug(std::move(vgraph));

	auto slicerView = std::make_unique<medViewSlicer>(_R("Slicer"));
	slicerView->PackageView();
	m_Logic->Plug(std::move(slicerView), view_visibility);

	auto igraph = std::make_unique<lhpViewInfo>(_R("Info view"));
	igraph->PlugVisualPipe(_R("lhpVMEKMInfo"), _R("lhpPipeInfo"));
	m_Logic->Plug(std::move(igraph));
	//temporary for testing
