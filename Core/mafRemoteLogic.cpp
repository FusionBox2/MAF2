...
//----------------------------------------------------------------------------
{
  ...
  else
  {
    ...
    if(command == "VIEW_CREATE")
    {
      ...
      {mafEvent evUnq(this,VIEW_CREATE,(intptr_t)(v_id + VIEW_START)); InvokeEvent(evUnq);}
      if (m_RemoteMouse)
      {
    ...
    else if(command == "SynchronizeView")
    {
      ...
      {
        m_ViewManager->m_FromRemote = true;
        {mafEvent evUnq(this,VIEW_CREATE,(intptr_t)v_id); InvokeEvent(evUnq);}
        m_ViewManager->m_FromRemote = false;
      }
    ...
    else if(command == "VIEW_DELETE")
    {
      ...
      if (m_RemoteMouse) 
      {
        {mafEvent evUnq(this,VIEW_DELETE,view); m_RemoteMouse->OnEvent(&evUnq);}
      }
    }
    else if(command == "VIEW_SELECTED")
    {
      ...
      if(view != NULL)
      {
        ...
        if (m_RemoteMouse) 
        {
          {mafEvent evUnq(this,VIEW_SELECT,view); m_RemoteMouse->OnEvent(&evUnq);}
        }
      }
      m_ViewManager->m_FromRemote = false;
    }
    ...
    else if(command == "VME_SELECT")
    {
      ...
      {mafEvent evUnq(this,VME_SELECT,(intptr_t)vme_id); InvokeEvent(evUnq);}
    }
    else if(command == "VME_SHOW")
    {
      ...
      {mafEvent evUnq(this,VME_SHOW, vme_to_show, show_flag != 0); InvokeEvent(evUnq);}
    ...
    else if (command == "OpenFile")
    {
      ...
      {mafEvent evUnq(this, MENU_FILE_OPEN, &fileToOpen); InvokeEvent(evUnq);}
    }
  }
}
//----------------------------------------------------------------------------
