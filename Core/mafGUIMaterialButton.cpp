...
void mafGUIMaterialButton::SetVME(mafVME *vme)
{
	...
	if(m_Vme)
	{
		m_Material = mmaMaterial::SafeDownCast(m_Vme->GetAttribute(_R("MaterialAttributes")));
		if (!m_Material)
		{
      m_Material = mmaMaterial::NewSPtr();
...
//----------------------------------------------------------------------------
{
  if (mafEvent *e = mafEvent::SafeDownCast(maf_event))
    ...
    {
      case ID_MATERIAL:
        {mafEvent evUnq(this,VME_CHOOSE_MATERIAL,m_Vme); InvokeEvent(evUnq);}
        m_MaterialLabel->SetLabel(m_Material->m_MaterialName.toWx());
        UpdateMaterialIcon();
