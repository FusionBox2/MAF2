...
//----------------------------------------------------------------------------
mafCxxTypeMacro(lhpOpImporterPressionCenter);
//----------------------------------------------------------------------------

//----------------------------------------------------------------------------
lhpOpImporterPressionCenter::_InternalPCData::_InternalPCData()
//----------------------------------------------------------------------------
{
  //vmes
  m_VmeGroup = NULL;
...
//----------------------------------------------------------------------------
lhpOpImporterPressionCenter::lhpOpImporterPressionCenter(const mafString& label) : Superclass(label)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
lhpOpImporterPressionCenter::~lhpOpImporterPressionCenter()
//----------------------------------------------------------------------------
{
  Clear();
}
...
//----------------------------------------------------------------------------
void lhpOpImporterPressionCenter::Clear()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
bool lhpOpImporterPressionCenter::Accept(mafNode *node)
//----------------------------------------------------------------------------
{
  return true;
}
//----------------------------------------------------------------------------
mafOp* lhpOpImporterPressionCenter::Copy()
//----------------------------------------------------------------------------
{
	lhpOpImporterPressionCenter *cp = new lhpOpImporterPressionCenter(GetLabel());
...
//----------------------------------------------------------------------------
void lhpOpImporterPressionCenter::OpRun()
//----------------------------------------------------------------------------
{
  mafString wildcard = "txt files (*.txt)|*.txt";

  m_PCInputFileNameFullPaths.clear();
  {
    mafGetOpenMultiFiles(m_FileDir,wildcard, m_PCInputFileNameFullPaths);
  }
  ///
...
//----------------------------------------------------------------------------
int lhpOpImporterPressionCenter::OpenPC(const mafString &fullFileName)
//----------------------------------------------------------------------------
{
  mafLogMessage("PressionCenter_Open");
...
//----------------------------------------------------------------------------
/*bool lhpOpImporterPressionCenter::LoadDictionary()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::DestroyDictionary()
//----------------------------------------------------------------------------
{
  m_dictionaryStruct.clear();
}*/
//----------------------------------------------------------------------------
/*bool lhpOpImporterPressionCenter::LoadLMRename()
//----------------------------------------------------------------------------
{
  vcl_string landmarkName, segmentName;
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::DestroyLMRename()
//----------------------------------------------------------------------------
{
  m_LMRenameStruct.clear();
}*/
//----------------------------------------------------------------------------
void lhpOpImporterPressionCenter::Initialize(const mafString &fullFileName, _InternalPCData &intData)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
	...
	if (OpenPC(fullFileName) == NOERROR)
	{
		...
		while (std::getline(infile, line))
		{
			...
			x = tokens[1]; y = tokens[2]; pression = tokens[3]; LR = tokens[4];
			intData.index = idx;
			intData.m_CenterX = x;
			intData.m_CenterY = y;
			intData.m_Time = time;
			intData.m_pression = pression;
			intData.m_LR = LR;
	...
	//process the data
	double* cumulIntensity, *nbr, *cumulXIntensity, *cumulYIntensity,*sumX,*sumY;
	cumulIntensity = new double[idx]{0};
	cumulYIntensity = new double[idx]{0};
	cumulXIntensity = new double[idx]{0};
	nbr = new double[idx]{0};
	sumX = new double[idx]{0};
	sumY = new double[idx]{0};
	int i;
	for (int j = 0; j < m_intData.size();j++)
	{
			i=m_intData.at(j).index;
			nbr[i] += 1;
			cumulIntensity[i] = cumulIntensity[i] + m_intData.at(j).m_pression;
			cumulXIntensity[i] = cumulXIntensity[i] + m_intData.at(j).m_CenterX*m_intData.at(j).m_pression;
			cumulYIntensity[i] = cumulYIntensity[i] + m_intData.at(j).m_CenterY*m_intData.at(j).m_pression;
			sumX[i] = sumX[i] + m_intData.at(j).m_CenterX;
			sumY[i] = sumY[i] + m_intData.at(j).m_CenterY;
	}

	//end data processing
	//wxBusyInfo wait(std::to_string(idx).c_str());
	//Sleep(1000);
	std::ofstream outfile("outputfile.txt");
	for (int j = 0; j <idx; j++)
		outfile << cumulXIntensity[j] / cumulIntensity[j] << "\t" << cumulYIntensity[j] / cumulIntensity[j] << "\t" << sumX[j] / nbr[j] << "\t" << sumY[j] / nbr[j] << "\n";
...
//----------------------------------------------------------------------------
bool lhpOpImporterPressionCenter::Import()
//----------------------------------------------------------------------------
{
  bool result = false;
  Clear();
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::ImportTrajectories(lhpOpImporterPressionCenter::_InternalPCData &intData)
//----------------------------------------------------------------------------
{
  wxBusyInfo *wait;
  if(!m_TestMode)
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::ImportAnalog(lhpOpImporterPressionCenter::_InternalPCData &intData)
//----------------------------------------------------------------------------
{
  wxBusyInfo *wait;
  if(!m_TestMode)
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::ImportPlatform(lhpOpImporterPressionCenter::_InternalPCData &intData)
//----------------------------------------------------------------------------
{
  wxBusyInfo *wait;
  if(!m_TestMode)
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::ImportEvent(lhpOpImporterPressionCenter::_InternalPCData &intData)
//----------------------------------------------------------------------------
{
  //For every event
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::CreateGui()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::DictionaryUpdate()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::LMRenameUpdate()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
/*void lhpOpImporterPressionCenter::OnEvent(mafEventBase *maf_event)
//----------------------------------------------------------------------------
{
...
*/
//----------------------------------------------------------------------------
void lhpOpImporterPressionCenter::OpDo()
//----------------------------------------------------------------------------
{

	int t = m_intData.size();

  //wxBusyInfo wait("Please wait, create all VMEs in tree");
	//wxBusyInfo wait(std::to_string(t).c_str());
	//Sleep(1000);
  for(unsigned i = 0; i < m_intData.size(); i++)
  {
...
//----------------------------------------------------------------------------
void lhpOpImporterPressionCenter::OpUndo()
//----------------------------------------------------------------------------
{   
  for(unsigned i = 0; i < m_intData.size(); i++)
  {
