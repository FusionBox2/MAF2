...
//----------------------------------------------------------------------------
mafCxxTypeMacro(medOpRegisterClusters2);
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
medOpRegisterClusters2::medOpRegisterClusters2(const mafString& label) : Superclass(label)
//----------------------------------------------------------------------------
{
  m_OpType	= OPTYPE_OP;
  m_Canundo = true;
...
//----------------------------------------------------------------------------
medOpRegisterClusters2::~medOpRegisterClusters2( ) 
//----------------------------------------------------------------------------
{
	vtkDEL(m_Follower);
...
//----------------------------------------------------------------------------
mafOp* medOpRegisterClusters2::Copy()   
//----------------------------------------------------------------------------
{
	return new medOpRegisterClusters2(GetLabel());
}
//----------------------------------------------------------------------------
bool medOpRegisterClusters2::Accept(mafNode* node)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpRegisterClusters2::OnEvent(mafEventBase *maf_event)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpRegisterClusters2::OpDo()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int medOpRegisterClusters2::ExtractMatchingPoints(double time)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
double medOpRegisterClusters2::RegisterPoints(double currTime)
//----------------------------------------------------------------------------
{
  ...
  if(m_Registered == NULL )
	{
		mafString name = m_Target->GetName() + _R(" registered on ") + m_Source->GetName();
		mafNEW(m_Registered);
		m_Registered->DeepCopy(m_Target);
  ...
  for(int i = 0; i < m_PointsSource->GetNumberOfPoints(); i++)
  {
	...
	m_PointsTarget->GetPoint(i, coord);
    coord[3] = 1.0;

    //m_PointsTarget->GetPoint(i, target);
	m_PointsSource->GetPoint(i, target);
    //transform point
  ...
  mafMatrix *mat;
	mafNEW(mat);
	mat->Identity();
  m_Source->GetOutput()->GetAbsMatrix(*mat,currTime);  //modified by Marco. 2-2-2004
  vtkMatrix4x4::Multiply4x4(mat->GetVTKMatrix(),RegisterTransform->GetMatrix(),t_matrix);
  mafDEL(mat);
  
	int numLandmarks = m_Source->GetNumberOfVisibleLandmarks(currTime);
    ...
    //z = m_Registered->GetOutput()->GetMatrix();
      ...
      mafMatrix msrc;
      msrc.Identity();
      m_Target->GetOutput()->GetAbsMatrix(msrc);
      msrc.Invert();
...
//----------------------------------------------------------------------------
void medOpRegisterClusters2::OnChooseTargetVme(mafNode *vme)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpRegisterClusters2::OnChooseSurfaceVme(mafNode *vme)
//----------------------------------------------------------------------------
{
  if(!vme) // user choose cancel - keep everything as before
    return;
...
//----------------------------------------------------------------------------
void medOpRegisterClusters2::SetTarget(mafVMELandmarkCloud *target)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpRegisterClusters2::SetFollower(mafVMESurface *follower)
//----------------------------------------------------------------------------
{
  // added by Losi on 31/01/2011 to allow test OpDo method
