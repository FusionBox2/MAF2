...
//----------------------------------------------------------------------------
{
  ...
  if(auto landmark = mafVMELandmark::SafeDownCast(GetInput())) 
  {
    landmark->Modified();
    landmark->Update();
  }
...
//----------------------------------------------------------------------------
{
  switch(maf_event->GetId())
    ...
    case ID_SCALE_Z:
    {
      ...
      mafVME::StaticDownCast(GetInput())->SetAbsMatrix(m_NewAbsMatrix, m_CurrentTime);
      ...
      // update gui 
      m_GuiTransformTextEntries->SetAbsPose(*mafVME::StaticDownCast(GetInput())->GetOutput()->GetAbsMatrix());
    ...
    switch(e->GetId())
    {
      case ID_TRANSFORM: // from m_GuiTransformTextEntries
      {
        ...
        *absPose = *(e->GetMatrix());
        ...
        if (m_RefSysVME == mafVME::SafeDownCast(GetInput()).get())
        {      
...
//----------------------------------------------------------------------------
{
  ...
  m_GuiTransformMouse = new mafGUITransformMouse(mafVME::SafeDownCast(GetInput()), this);
  ...
  //---------------------------------
  // create the transform Gui
  m_GuiTransformTextEntries = new mafGUITransformTextEntries(mafVME::SafeDownCast(GetInput()), this,false);
