...
//----------------------------------------------------------------------------
{
  ...
  if(answer == wxYES)
  {
    ...
    median->SetInputConnection(m_VolumeInput->GetOutput()->GetVTKOutputPort());
    ...
    case ID_REGION_GROWING:
      {
        ...
        vtkNew<vtkImageToStructuredPoints> filter;
    ...
    case ID_MORPHOLOGICAL:
      {
        ...
        {
          connectivityFilter->SetInputConnection(extractIsosurface->GetOutputPort());
