...
//----------------------------------------------------------------------------
{
  ...
  wxString OutputFormats[1] = {"Vector"};
  wxStaticText *textureCtrlsStaticTxt  = new wxStaticText(m_Dialog, -1, "Texture Controls");
  wxStaticText *texWinSizeStaticTxt = new wxStaticText(m_Dialog, -1, "sample window size ") ;
  wxStaticText *texWinStepStaticTxt = new wxStaticText(m_Dialog, -1, "sample spacing ") ;
  ...
  wxComboBox *outputFormatCombo = new wxComboBox(m_Dialog, ID_OUTPUT_FORMAT_COMBO, OutputFormats[0], p, wxSize(80,20), 1, OutputFormats, wxCB_READONLY) ;
  ...
  texWinSizeValueTxt->SetValidator(mafGUIValidator(this, ID_TEXWINSIZE_VALUE_TXT, texWinSizeValueTxt, &m_texWinSize)) ;
  texWinStepValueTxt->SetValidator(mafGUIValidator(this, ID_TEXWINSTEP_VALUE_TXT, texWinStepValueTxt, &m_texWinStepSize)) ;
...
//----------------------------------------------------------------------------
{
  if (mafEvent *e = mafEvent::SafeDownCast(maf_event))
  {
    ...
    case ID_OUTPUT_FORMAT_COMBO:
      // change the output format (vector or tensor)
      m_vectorGlyphPipe->SetOutputFormat((OutputFormat)m_outputFormat) ;
      break ;

    case ID_FILENAME_TXT:
      // change the output filename (disable the print button when empty)
    ...
    case ID_PRINT_BUTTON:
      { 
        ...
        outputStream.open(mafStringToStd(mafWxToString(m_outputFilename)), fstream::out | fstream::app) ;
    ...
    case ID_UPDATE:   
      ...
      m_vectorGlyphPipe->SetVisibility(1) ; // make sure pipe is visible
      UpdateCamera() ;
      break ;
...
//----------------------------------------------------------------------------
{
  ...
  // This is a slow pipe, so we use SetVisibility(0) to stop it updating before the dialog is ready.
  if (m_vectorGlyphPipe == NULL){
