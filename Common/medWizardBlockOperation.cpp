...
//----------------------------------------------------------------------------
{
  ...
  if (m_RequiredView!="")
  {
	...
	if(m_windowhastoberesized) 
	{
	    ...
	    {mafEvent evUnq(this,VIEW_RESIZE); evUnq.SetString(&tmpStr); evUnq.SetX(x); evUnq.SetY(y); evUnq.SetWidth(width); evUnq.SetHeight(height); InvokeEvent(evUnq);}
	}
...
//----------------------------------------------------------------------------
{
  ...
  for(int i=0;i<m_VmeHide.size();i++)
  {
    ...
    {mafEvent evUnq(this,VME_SHOW); evUnq.SetVme(toHide); InvokeEvent(evUnq);}
  }
