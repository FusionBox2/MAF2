  ...
  for (int i=0;i<children.GetNumItems();i++)
  {
    //if (subnode->GetName() == _R("Device"))
    {
      ...
      //if (children[i].GetAttribute(_R("Name"),name) == MAF_OK)
      {
        ...
        {mafEvent evUnq(this,DEVICE_BIND); evUnq.SetArg(id); InvokeEvent(evUnq);}
      }
//       else
//       {
  ...
  if (ch==MCH_OUTPUT && id==QUERY_CONNECTED_DEVICES)
  {
    ...
    for (auto it=m_Devices.begin();it!=m_Devices.end();++it)
    {
      ...
      if (dev->IsInitialized())
      {
        ...
        {mafEventBase evUnq(this,DEVICE_PLUGGED); evUnq.SetData(dev); evUnq.SetChannel(MCH_INPUT); sender->OnEvent(&evUnq);}
      }      
    }
  }
