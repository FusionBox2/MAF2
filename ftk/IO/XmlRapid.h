...
namespace io::xmlrapid
{
	...
	private:
		...
		Value(const std::vector<void*>& c, void* attr, std::shared_ptr<NativeHolder>);

		std::shared_ptr<NativeHolder> m_holder;
		std::vector<void*> m_current;
		void* m_attr = nullptr;
		std::map<std::string, std::vector<void*> > m_children;
	};
...
namespace io::xmlrapid
{
	...
	void ValueBuilder::setValue(const T& t)
	{
		static_assert(
			...
			"There is no `Serialize(xmlrapid::ValueBuilder&, const T&, io::serialize::To<xmlrapid::ValueBuilder>)` "
			"in namespace of `T` or `io::serialize`. "
			"Probably you have not provided a `Serialize` function overload."
			);
		Serialize(*this, t, io::serialize::To(*this));
	}
