...
namespace  model::data_
{
  ...
  MTimeType Node::GetMTime()
  {
	...
	void Node::Store(mafStorageElementBuilder& builder) const
  {
  ...
  void Node::InternalStore(mafStorageElementBuilder& builder) const
  {
    builder(_R("Name")).SetValue(m_Name);
    builder(_R("Id")).SetValue(m_Id);
  ...
  void Node::InternalRestore(const mafStorageElement& value)
  {
    ...
    //RemoveAllAttributes();
    {
      auto attributes = value[_R("Attributes")];
      auto numItemsAttr = attributes(_R("NumberOfItems")).As<mafID>();
      auto attr_items = attributes[_R("Item")];
      ...
      //m_Attributes = attr_items.As<decltype(m_Attributes)>();
    ...
    {
      if (auto item = attr_items[i].As<mafAttribute>())
      {
        m_Attributes.insert(std::move(item));
      }
      else
      {
        assert(false);
    ...
    {
      auto links = value[_R("Links")];
      auto numLinks = links(_R("NumberOfLinks")).As<mafID>();
      auto links_vector = links[_R("Link")];
      if (numLinks != links_vector.size())
      {
        mafErrorMacro("Number of links differs from number of entries");// << GetName());
        return;
      }

      m_Links = links_vector.As<decltype(m_Links)>();

      //links_vector = value[_R("Links")][_R("Link")].As<decltype(m_Links)>();
      for (size_t i = 0; i < links_vector.size(); i++)
      {
        ...
        {
          //m_Links[link_name] = NodeLink(nullptr, link_node_subid).SetId(link_node_id);
        }
      }
    }
    // restore Links
    ...
    // restore children
    {
      auto children = value[_R("Children")];
      auto numItemsChild = children(_R("NumberOfItems")).As<mafID>();
      auto children_nodes = children[_R("Node")];
      if (numItemsChild != children_nodes.size())
      {
        mafErrorMacro("Number of children differs from number of entries");// << GetName());
        return;
      }

      //m_Children = children_items.As<decltype(m_Children)>();
    ...
    {
      auto value = child_items[i].As<Node>();
      assert(value);
      if (value)
      {
        value->m_Parent = this;
...
//-------------------------------------------------------------------------
std::shared_ptr<Node> Node::GetFirstChild(bool onlyVisible /*=false*/)
//-------------------------------------------------------------------------
{
  for (auto& child : m_Children)
  {
...
//-------------------------------------------------------------------------
std::shared_ptr<Node> Node::GetLastChild(bool onlyVisible /*=false*/)
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
{
  ...
  auto iter = std::make_unique<NodeIterator>(sharedThis.get());
...
//-------------------------------------------------------------------------
{
  ...
  {
    auto iter = std::make_unique<NodeIterator>(this);
    for (auto n = iter->GetFirstNode(); n; n = iter->GetNextNode())
    {
  ...
  if(res == nullptr)
    return nullptr;
  {
    auto iter = std::make_unique<NodeIterator>(res.get());
    unsigned i = 0;
...
//-------------------------------------------------------------------------
void Node::SetAttribute(const mafString& name,std::shared_ptr<mafAttribute> a)
//-------------------------------------------------------------------------
{
  m_Attributes[name]=a;
}
