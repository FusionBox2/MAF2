...
namespace meta
{
	...
	template <typename T>
	inline constexpr bool isCharacter = std::is_same_v<T, char> || std::is_same_v<T, wchar_t> || std::is_same_v<T, char16_t> || std::is_same_v<T, char32_t> || std::is_same_v<T, char8_t>;
	...
	namespace impl
	{
		...
		template <typename T>
		using IsRange = ExpectSame<std::decay_t<decltype(begin(std::declval<T&>()))>, std::decay_t<decltype(end(std::declval<T&>()))>>;
	...
	template <typename T>
	inline constexpr bool kIsMap = isDetected<impl::IsRange, T>&& isDetected<impl::KeyType, T>&& isDetected<impl::MappedType, T>;

	/// Returns true if T is a map (but not a multimap!)
	template <typename T>
	inline constexpr bool kIsUniqueMap = kIsMap<T> && isDetected<impl::SubscriptOperatorResult,T>;// no operator[] in multimaps
}
