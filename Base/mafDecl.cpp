...
//----------------------------------------------------------------------------
std::vector<mafString> mafGetOpenMultiFiles(const mafString& initial, const mafString& wild, const mafString& title, wxWindow* parent)
//----------------------------------------------------------------------------
{
	std::vector<mafString> files;
	wxString path, name, ext;
	wxFileName::SplitPath(mafStringToWx(initial), &path, &name, &ext);
	if (!name.IsEmpty() && !ext.IsEmpty()) name.Append(".").Append(ext);
	mafString wildcard = wild;
